...
RSpec.describe BulkDiscount, type: :model do
  it { should belong_to(:merchant) }
	...
	describe 'applied invoices' do
		it 'returns only invoices that the bulk discount can be applied to' do
			...
			invoice_item_c = create(:invoice_item, invoice: invoice_b, item: item_b, quantity: 15, unit_price: 5)
			invoice_item_c = create(:invoice_item, invoice: invoice_b, item: item_a1, quantity: 5, unit_price: 5)
			...
			expect(bd_1.applied_invoices).to eq([invoice_a])
		end
	end
